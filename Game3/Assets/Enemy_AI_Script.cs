...
public class Enemy_AI_Script : MonoBehaviour
{
    ...
    {
        // Subscribe to the dropSound event
...
{
    // -------------------------
    // 1️⃣ Player has highest priority
    // -------------------------
    if (playerIsTracked)
    {
        investigatingSound = false; // Stop investigating any sounds
        patrolling = false;
        ...
        //Assign 12f as the speed while chasing the player
        agent.speed = 12f;
        return; // Skip everything else while chasing player
    }

    // -------------------------
    // 2️⃣ Investigate sounds if not chasing player
    // -------------------------
    if (investigatingSound)
    {
        ...
        agent.speed = 12f;
        return; // Skip patrolling while investigating
    }
    ...
    void HandleMovement()
    {
        if (patrolling && !playerIsTracked && !investigatingSound)
        {
            agent.speed = 7f;
        ...
        // Path blocked → try door interaction
    ...
    void EvaluateDoorVsPath()
    {
        ...
        float pathWithDoorsOpen = GetPathLength(player.position); // Simplified for now
...
{
    // Only investigate sounds if NOT chasing the player
